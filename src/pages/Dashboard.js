...
export default function Dashboard(props) {
    ...
    const loadUsers=async()=>{
       ...
       const ele = <h3 style={{marginTop:"20px"}} class="display-6">Hello, {result.data[0].name}</h3>
       root.render(ele);
    }

     const handleTransHistoryClick = async () =>{
      try {
         const account = await axios.get("http://localhost:4005/accounts?userId="+props.id)
         setAccount(account)
         console.log(account);
         const root = ReactDOM.createRoot(
        ...
        const element = (
           ...
           <h4>Transaction history: {JSON.stringify(account.data[0].transactionHistory)}</h4>
           </div>
        )
        root.render(element);
     ...
     const handleAccDetailsClick = async () => {
      try {
          const user = await axios.get("http://localhost:4000/users?id="+props.id)
          setData(user)
          const account = await axios.get("http://localhost:4005/accounts?userId="+props.id)
          setAccount(account)
          const root = ReactDOM.createRoot(
...
<div className = 'py-4'>
    ...
    <div class="container">
      <div class="row">
         <div class="col-6">
            <div id="namediv" style={{float:"left"}}>
            </div>
         </div>
         <div class="col-6">
      ...
      <div style={{float:"left"}}>
         ...
         <button style={{width:"250px",marginRight:"5px"}}type="button" onClick={handleAccDetailsClick} class="btn btn-dark">Account Details</button>
         <button style={{width:"250px",marginRight:"5px"}}type="button" class="btn btn-dark" onClick={handleTransHistoryClick}>Transaction history</button>
